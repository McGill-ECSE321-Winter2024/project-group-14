...
function hashPassword(password) {
    ...
    export default {
      ...
      methods:{
        findByName(name){
            AXIOS.get(`/customer/${name}`).then(response => {        
        ...
        delete(){
            ...
            .then(() => {
            const index = this.tableData.findIndex(customer => customer.username === this.username);
            if (index !== -1) {
        ...
        save() {
            AXIOS.post(`/customer/`, null, {
                ...
                .catch((e) => {
                this.error = e;
                this.$message.error('Error adding new customer: Please input correct parameters');
                });
            },
        ...
        .then((response) => {
            const index = this.tableData.findIndex(customer => customer.username === this.form.oldUsername);
            if (index !== -1) {
        ...
        .catch((e) => {
            this.error = e;
            this.$message.error('Error editing customer: Please input correct parameters');
        });
